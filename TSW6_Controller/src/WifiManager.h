...
"});"
  ...
  html += "<div class='hint'>Erreiche mich auch unter "
          "<a href='http://" + String(MDNS_NAME) + ".local'>" + String(MDNS_NAME) + ".local</a></div>";
  html += "</div></body></html>";
  return html;
}
...
void buildSetupForm(String& out) {
  ...
  for (int i = 0; i < n; i++) {
    ...
    ss.replace("&","&amp;"); ss.replace("<","&lt;"); ss.replace(">","&gt;");
    out += "<option value='" + ss + "'";
    if (strlen(cfg.ssid) && ss == String(cfg.ssid)) out += " selected";
    out += ">" + ss + "</option>";
  }
  out += "</select>";
...
void handleStatusPage() {
 ...
 body += "<img src='/logo_Ole_TSW.svg' alt='TSW Logo'>";
  ...
  String currSsid = WiFi.SSID(); if (!currSsid.length()) currSsid = cfg.ssid;
...
void startAPMode() {
  ...
  Serial.println("Webserver läuft auf http://192.168.4.1 sowie http://TSWController.local");
}
...
void beginWiFiManager() {
  ...
  if (setupPressed) {
    ...
    if (cfg.apModePreferred) startAPMode();
